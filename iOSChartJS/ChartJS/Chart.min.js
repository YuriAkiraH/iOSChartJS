...
var jsonString = document.getElementById('json').value; //recebe os dados em json para criar o grafico

function parse(json) { //function to convert into a obj (UIWebView does not support JSON.parse())
    return eval('(' + json + ')');
}

var objJSON = parse(jsonString); //converting into obj

var arrayName = [];
var arrayQuantity = [];
var arrayColor = [];

for (var i = 0; i < objJSON.length; i++) { //array that hold data
    arrayName[i] = objJSON[i].name;
    arrayQuantity[i] = objJSON[i].quantity;
    arrayColor[i] = objJSON[i].color;
}
